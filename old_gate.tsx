...
export default function GateOperationsPage() {
  ...
  const handleGateIn = async () => {
    if (!form.numero_conteneur || !form.emplacement_id) {
      toast.error("Veuillez remplir le num├®ro de conteneur et choisir un emplacement.");
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast.success("Gate In valid├® avec succ├¿s !");
      setScanResult(null);
      setFile(null);
      setIsManual(false);
      setForm({
  ...
  const handleGateOut = async () => {
    if (!form.numero_conteneur) {
      toast.error("Veuillez remplir le num├®ro de conteneur.");
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast.success("Gate Out valid├® avec succ├¿s !");
      setScanResult(null);
      setFile(null);
      setIsManual(false);
      setForm({
      ...
      <div className="max-w-4xl mx-auto py-8 px-4 animate-in fade-in duration-500">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-2">Reconnaissance optique des documents d'entr├®e/sortie du parc.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-slate-900 rounded-3xl p-8 shadow-xl text-white relative overflow-hidden">
            ...
            <h2 className="text-lg font-bold mb-6 flex items-center gap-2 relative z-10">
              ...
              {isManual ? `Validation Gate ${mode}` : "R├®sultats de l'Extraction"}
            </h2>

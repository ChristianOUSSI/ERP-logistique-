...
def test_valider_creation_mission_non_autorise_transport(mock_db_session):
    """Vérifie que la création échoue si le client n'est pas autorisé pour le transport."""
    ...
    mock_client = Tiers(id=1, raison_sociale="Client Test", autorise_transport=False)
    
    with patch('app.services.tiers_service.TiersService.get_tiers', return_value=mock_client):
        # WHEN / THEN
        with pytest.raises(ValueError, match="Le service Transport n'est pas autorisé"):
            MissionTransportService.valider_creation_mission(mock_db_session, mission_data)


def test_valider_creation_mission_credit_limite_depasse(mock_db_session):
    """Vérifie que la création échoue si la limite de crédit du client est dépassée."""
    from app.models.tiers import Tiers
    ...
    mock_client = Tiers(id=1, raison_sociale="Client Test", autorise_transport=True)
    mock_encours = {
        "limite_credit_xaf": Decimal("500000"),
        "encours_xaf": Decimal("600000")
    }
    
    with patch('app.services.tiers_service.TiersService.get_tiers', return_value=mock_client):
        with patch('app.services.finance_service.EncoursService.calculer_encours_client', return_value=mock_encours):
            # WHEN / THEN
            with pytest.raises(ValueError, match="Limite de crédit dépassée"):
                MissionTransportService.valider_creation_mission(mock_db_session, mission_data)
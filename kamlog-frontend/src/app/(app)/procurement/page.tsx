...
export default function ProcurementPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Bon de commande (PO) émis avec succès !');
  ...
  if (!mounted) return <div className="p-8 text-center text-slate-500">Chargement du module K-Procurement...</div>;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-500/10 text-violet-400 text-xs font-semibold mb-2 border border-violet-500/20">
            ...
            K-Procurement • Approvisionnements & Matching 3-Voies
          </div>
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight">Achats & Bons de Commande (PO)</h1>
          <p className="text-slate-400 text-sm mt-1">Gestion des commandes fournisseurs, pièces détachées et réceptions d'atelier.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-300">
            <thead className="bg-slate-950 text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-800">
              <tr>
                <th className="px-6 py-4">N° PO / Fournisseur</th>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {isLoading ? (
                ...
                <tr><td colSpan={4} className="p-8 text-center text-slate-500">Aucune commande enregistrée.</td></tr>
              ) : (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg p-6 text-white shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleCreate} className="space-y-4 pt-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                ...
                >
                  {createMutation.isPending ? 'Émission...' : 'Valider la Commande'}
                </button>
              </div>
            </form>
          </div>

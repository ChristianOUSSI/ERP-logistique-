import { ModuleLayout } from '@/components/layout/ModuleLayout';

export default function NotificationSettingsEscalationPage() {
  return (
    <ModuleLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-headline-lg text-headline-lg text-on-surface">
            Security: Notification Settings & Escalation
          </h1>
          <p className="text-body-md text-body-md text-on-surface-variant mt-1">
            Configuration des alertes de sécurité et politiques d'escalade
          </p>
        </div>
        <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-6">
          <p className="text-body-md text-on-surface-variant">
            Interface de configuration des notifications - En cours de développement
          </p>
        </div>
      </div>
    </ModuleLayout>
  );
}

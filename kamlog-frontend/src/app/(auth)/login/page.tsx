// src/app/(auth)/login/page.tsx - ERP Design - Fidèle 100% au HTML original
'use client'
...
// ── Page Login ERP Design - Fidèle 100% au HTML original ─────────────
export default function LoginPage() {
  ...
  useEffect(() => {
    // Simple interactive micro-interactions comme dans le HTML original
    const inputs = document.querySelectorAll('input')
    inputs.forEach((input) => {
      const handleFocus = () => {
        const label = input.parentElement?.parentElement?.querySelector('label') as HTMLElement
        if (label) {
  ...
  // ── Rendu ERP Design - Fidèle 100% au HTML original ─────────────
  return (
    <>
      <style jsx global>{`
      ...
      <div className="bg-surface-container-low text-on-surface font-body-md min-h-screen flex items-center justify-center overflow-hidden relative">
        ...
        <div className="absolute inset-0 z-0">
          ...
          {/* Background Imagery (Subtle Logistical Mood) */}
          <div className="absolute inset-0 opacity-10 pointer-events-none">
            <img 
              className="w-full h-full object-cover" 
              data-alt="A sophisticated wide shot of a modern automated shipping terminal with blue cargo containers stacked in precise rows under a bright daylight sky. The scene conveys industrial precision and large-scale logistics efficiency, reflecting the core brand identity of KAMLOG EM-ERP. The lighting is crisp and even, emphasizing a professional corporate atmosphere with subtle blue and grey tones." 
          ...
          <div className="bg-white border border-outline-variant rounded-lg p-lg auth-card">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-md">
              ...
              <div className="space-y-xs">
                <label className="text-label-md font-label-md text-on-surface-variant" htmlFor="email">Institutional Email</label>
              ...
              <div className="space-y-xs">
                <div className="flex justify-between items-center">
                  <label className="text-label-md font-label-md text-on-surface-variant" htmlFor="password">Security Key</label>
                  <a className="text-label-md font-label-md text-auth-blue hover:underline" href="#">Forgot password?</a>
                </div>
                <div className="relative">

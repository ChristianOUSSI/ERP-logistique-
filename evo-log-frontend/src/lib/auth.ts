...
const ALL_MODULES = [
  ...
  "dashboard", "rh", "acconage", "port-operations", "qhse", "transit", "maintenance", "client-portal",
...
const PREDEFINED_USERS: Record<string, { id: string; nom_complet: string; role: string; roles: string[]; modules_allowed: string[] }> = {
  ...
  'douane@evo-log.cm': { id: 'usr-006', nom_complet: 'D̩clarant en Douane & Transit', role: 'DOUANE', roles: ['DOUANE', 'TRANSIT'], modules_allowed: ['transit', 'master-data', 'acconage'] },
  ...
  'maintenance@evo-log.cm': { id: 'usr-008', nom_complet: 'Ing̩nieur Maintenance Garages', role: 'MAINTENANCE', roles: ['MAINTENANCE'], modules_allowed: ['maintenance', 'parc'] },
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          try {
            ...
            console.warn("Connexion serveur distant indisponible, bascule sur la session certifi̩e localement.", apiErr);

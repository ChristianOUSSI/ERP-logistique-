...
export default function ModuleSidebar({
  ...
  // Obtention de la navigation dynamique filtrÃ©e selon le rÃ´le RBAC de l'utilisateur
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-slate-900 border-r border-slate-800 text-slate-300 w-full select-none">
          ...
          {!isCollapsed && (
            <div>
              ...
              <span className="text-[10px] text-slate-400 font-mono block">
                {isAdmin ? "AccÃ¨s Admin Total" : `Profil : ${userRoles[0] || 'Utilisateur'}`}
              </span>
            </div>
          )}
        </div>
        ...
        {isMobile ? (
          ...
          onToggle && (
            <button
              ...
              title={isCollapsed ? "DÃ©plier la Sidebar" : "RÃ©tracter la Sidebar"}
            >
          ...
          if (!isAllowed) {
            return (
              <div
                ...
                title={`Module ${item.title} non inclus dans votre profil. Cliquez pour voir les dÃ©tails.`}
          ...
          >
            {isCollapsed ? (
              ...
              <>
                ...
                <span>RÃ©tracter la sidebar</span>
              </>
            )}
          </button>
        </div>
      ...
      {/* ðŸ”’ Modale d'avertissement Module Non AutorisÃ© */}
      {deniedModalItem && (
          ...
          <div className="bg-slate-900 border border-amber-500/40 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4 animate-in zoom-in-95 duration-200 text-slate-100">
            ...
            <div className="text-center space-y-2">
              <span className="text-[10px] font-black tracking-widest text-amber-400 uppercase bg-amber-500/10 px-3 py-1 rounded-full border border-amber-500/20 inline-block">
                AccÃ¨s Restreint CADC
              </span>
              <h3 className="text-lg font-black">Module Non AutorisÃ©</h3>
              <p className="text-xs text-slate-300 leading-relaxed font-medium bg-slate-950 p-3 rounded-2xl border border-slate-800">
                AccÃ¨s restreint : Votre profil <b className="text-amber-400">[{userRoles.join(", ") || "Utilisateur"}]</b> n'est pas autorisÃ© Ã  accÃ©der au module <b className="text-white">[{deniedModalItem.label}]</b>. Veuillez contacter l'Admin CADC.
              </p>
              <div className="p-3 bg-slate-950/80 border border-slate-850 rounded-xl text-left text-xs font-mono text-slate-400 space-y-1">
                <div>â€¢ Code module : <span className="text-amber-400">{deniedModalItem.key}</span></div>
                <div>â€¢ Vos modules autorisÃ©s : <span className="text-slate-200">{userModules.length > 0 ? userModules.join(", ") : "Aucun"}</span></div>
              </div>
            </div>
